...
const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
  ...
  const handleDeleteScenario = async (e: React.MouseEvent, scenario: Scenario) => {
    ...
    const targetId = scenario.internalId || scenario.id;
    if (!targetId) {
      alert("識別IDが見つかりません。一度同期を行ってください。");
      return;
    }

    if (!window.confirm(`「${scenario.name}」を完全に削除しますか？`)) return;
    
    // Fix: deleteItem expects 3 arguments, removed the 4th argument (headers array)
  ...
  const handleDeleteTest = async (e: React.MouseEvent, question: TestQuestion) => {
    ...
    const targetId = question.internalId || question.id;
    if (!targetId) {
      alert("識別IDが見つかりません。");
      return;
    }
    ...
    <div className="space-y-4 animate-fade-in text-left pb-10">
      ...
      {isScenarioModalOpen && (
        <ScenarioEditorModal 
          ...
          onDelete={editingScenario ? (e) => handleDeleteScenario(e, editingScenario as Scenario) : undefined}
      ...
      {isTestModalOpen && (
        <TestQuestionEditorModal
          ...
          onDelete={editingTest ? (e) => handleDeleteTest(e, editingTest) : undefined}
