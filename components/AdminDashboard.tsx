...
const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
  ...
  const handleDeleteScenario = async (e: React.MouseEvent, scenario: Scenario) => {
    e.preventDefault();
    e.stopPropagation(); // これが tr へのクリック伝播を防ぐ鍵

    if (!scenario.internalId) {
      alert("この項目はまだ同期されていません。一度同期ボタンを押してください。");
      return;
    }

    if (!window.confirm(`「${scenario.name}」を完全に削除しますか？\n(スプレッドシートからも即座に削除されます)`)) return;
    
    const next = await deleteItem(scenario.internalId, props.scenarios, 'シナリオ', ["ID", "シナリオ名", "センター", "スマホプラン", "光プラン", "最初の問い合わせ内容", "難易度", "性質"]);
    if (next) {
  ...
  const handleDeleteTest = async (e: React.MouseEvent, question: TestQuestion) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    if (!window.confirm(`問題「${question.name}」を完全に削除しますか？`)) return;
    
    const next = await deleteItem(question.internalId, props.testQuestions, 'テスト問題', ["ID", "テスト名", "センター", "問題文", "解答", "難易度"]);
    if (next) {
          ...
          {activeTab === 'scenarios' && (
            <div className="space-y-3 animate-fade-in">
              <div className="flex justify-between items-center px-1">
                ...
                <button onClick={() => { setEditingScenario(null); setIsScenarioModalOpen(true); }} className="bg-slate-900 text-white px-3 py-1 rounded-lg font-black flex items-center gap-1.5 hover:bg-black transition-all shadow-sm text-[9px] cursor-pointer h-7">
              ...
              <div className="bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm">
                <table className="w-full text-[12px] text-left border-collapse">
                  <thead className="bg-slate-50 text-slate-400 font-black uppercase tracking-widest border-b border-slate-100">
                    <tr>
                      <th className="px-5 py-1.5 w-14 text-center text-[9px]">No</th>
                      <th className="px-5 py-1.5 w-32 text-[9px]">管理ID</th>
                      <th className="px-5 py-1.5 border-l border-slate-100 text-[9px]">所属</th>
                      <th className="px-5 py-1.5 border-l border-slate-100 text-[9px]">シナリオ名</th>
                      <th className="px-5 py-1.5 w-24 text-center border-l border-slate-100 text-[9px]">難易度</th>
                      <th className="px-5 py-1.5 w-20 text-right text-[9px]">操作</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {props.scenarios.map((s, idx) => (
                      <tr key={s.internalId || idx} className="hover:bg-slate-50/50 transition-colors group cursor-pointer" onClick={() => { setEditingScenario(s); setIsScenarioModalOpen(true); }}>
                        <td className="px-5 py-0.5 text-slate-400 font-bold text-center h-8 align-middle">{idx + 1}</td>
                        <td className="px-5 py-0.5 font-mono text-slate-400 truncate max-w-[120px] h-8 align-middle">{s.id || '-'}</td>
                        <td className="px-5 py-0.5 border-l border-slate-100 h-8 align-middle">
                        ...
                        <td className="px-5 py-0.5 flex justify-center border-l border-slate-100 items-center h-8 align-middle">{renderStars(s.difficulty || 0)}</td>
                        <td className="px-5 py-0.5 text-right h-8 align-middle">
                          <div className="flex justify-end gap-2 pr-1">
                            <button 
                                type="button"
                                onClick={(e) => handleDeleteScenario(e, s)} 
                                className="p-2 -m-1 text-slate-300 hover:text-rose-600 transition-colors cursor-pointer z-10"
                                title="削除"
                            >
          ...
          {activeTab === 'tests' && (
             <div className="space-y-3 animate-fade-in">
                <div className="flex justify-between items-center px-1">
                  ...
                  <button onClick={() => { setEditingTest(null); setIsTestModalOpen(true); }} className="bg-slate-900 text-white px-3 py-1 rounded-lg font-black flex items-center gap-1.5 hover:bg-black transition-all shadow-sm text-[9px] cursor-pointer h-7">
                  ...
                  <table className="w-full text-[12px] text-left border-collapse">
                    <thead className="bg-slate-50 text-slate-400 font-black uppercase tracking-widest border-b border-slate-100">
                      <tr>
                        ...
                        <th className="px-5 py-1.5 w-32 text-[9px]">管理ID</th>
                        <th className="px-5 py-1.5 border-l border-slate-100 text-[9px]">所属</th>
                        <th className="px-5 py-1.5 border-l border-slate-100 text-[9px]">問題タイトル</th>
                        <th className="px-5 py-1.5 w-24 text-center border-l border-slate-100 text-[9px]">難易度</th>
                        <th className="px-5 py-1.5 w-20 text-right text-[9px]">操作</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {props.testQuestions.map((q, idx) => (
                        <tr key={q.internalId || idx} className="hover:bg-slate-50/50 transition-colors group cursor-pointer" onClick={() => { setEditingTest(q); setIsTestModalOpen(true); }}>
                          <td className="px-5 py-0.5 text-slate-400 font-bold text-center h-8 align-middle">{idx + 1}</td>
                          <td className="px-5 py-0.5 font-mono text-slate-400 truncate max-w-[140px] h-8 align-middle">{q.id || '-'}</td>
                          <td className="px-5 py-0.5 border-l border-slate-100 h-8 align-middle">
                          ...
                          <td className="px-5 py-0.5 flex justify-center border-l border-slate-100 items-center h-8 align-middle">{renderStars(q.difficulty || 0)}</td>
                          <td className="px-5 py-0.5 text-right h-8 align-middle">
                            <div className="flex justify-end gap-2 pr-1">
                              <button 
                                type="button"
                                onClick={(e) => handleDeleteTest(e, q)} 
                                className="p-2 -m-1 text-slate-300 hover:text-rose-600 transition-colors cursor-pointer z-10"
                                title="削除"
                              >
                                <TrashIcon className="h-4 w-4" />
