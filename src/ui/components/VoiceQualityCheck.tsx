...
import { CheckCircleIcon, LoadingIcon, MicIcon, PlayIcon, WifiOffIcon } from './Icons'
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onerror: (e) => {
            console.error(e)
            setErrorMessage(
              'WebSocket接続エラー: ネットワークにより遮断されている可能性があります。',
            )
            setStatus('failed')
          },
          onclose: () => {
        ...
        config: {
          ...
          systemInstruction:
            'あなたは接続テスト用のAIです。短く元気に「接続テスト成功です。通信品質は良好です。」と答えてください。',
        },
      })
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      <div className="bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
            ...
            <div className="flex flex-col items-center gap-4 text-rose-600 px-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-rose-800 mt-2 bg-white p-3 rounded-lg border border-rose-200">
                  {errorMessage ||
                    '通信経路上のセキュリティ設定により、リアルタイム通信が制限されています。'}
                </p>
              </div>
              <button
                onClick={runDiagnostic}
                className="mt-4 bg-rose-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-rose-700 shadow-md"
              >
                再試行
              </button>
            </div>
        ...
        <div className="bg-slate-900 rounded-lg p-4 font-mono text-[10px] text-sky-400 overflow-hidden shadow-inner">
          <p className="text-slate-500 mb-2 border-b border-slate-800 pb-1 uppercase tracking-widest font-bold">
            Diagnostic Logs
          </p>
