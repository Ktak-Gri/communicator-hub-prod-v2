...
const TestResultModal: React.FC<TestResultModalProps> = ({ result, onClose }) => {
  ...
  return (
    <div
      className="fixed inset-0 bg-black bg-opacity-60 z-[110] overflow-y-auto p-4 sm:p-8 animate-fade-in"
      onClick={onClose}
    >
      <div
        role="dialog"
        aria-modal="true"
        className="bg-slate-50 rounded-3xl shadow-2xl w-full max-w-3xl my-24 mx-auto"
      ...
      >
        <div className="p-6 border-b flex justify-between items-center bg-white rounded-t-3xl">
          <div>
            ...
            <div className="flex mt-1">{renderStars(result.difficulty)}</div>
          </div>
        ...
        <div className="p-8 space-y-6">
          <div>
            ...
            <p className="p-4 bg-white rounded-xl text-slate-800 font-bold border border-slate-200">
              {result.questionText}
            </p>
          </div>
          <div>
            ...
            <p className="p-4 bg-sky-50 border border-sky-100 rounded-xl text-slate-800 font-bold whitespace-pre-wrap">
              {result.userAnswer}
            </p>
          </div>
          <div className="p-6 bg-emerald-50 border-l-4 border-emerald-400 rounded-r-xl">
            <div className="flex justify-between items-start">
              <div className="flex-1">
                ...
                <p className="text-emerald-900 text-sm font-bold leading-relaxed whitespace-pre-wrap">
                  {result.aiFeedback}
                </p>
              </div>
