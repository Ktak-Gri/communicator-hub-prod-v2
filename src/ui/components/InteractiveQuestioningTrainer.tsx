  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 animate-fade-in">
      <div className="flex flex-row items-center">
        <button
          onClick={onBack}
          className="flex flex-row items-center gap-1.5 text-xs font-black text-slate-400 hover:text-sky-600 transition-all whitespace-nowrap group px-2"
        >
        ...
        <div className="p-6 border-b border-slate-800/50 bg-slate-950/80 backdrop-blur-xl flex justify-between items-center z-20">
          ...
          {status === 'connected' && (
            <button
              ...
              className="bg-rose-600 hover:bg-rose-500 text-white px-6 py-2.5 rounded-xl font-black text-xs shadow-lg transition-all flex items-center gap-2 transform active:scale-95"
            >
          ...
          <div className="flex-1 flex flex-col relative overflow-hidden">
            {status === 'ringing' && (
              <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-slate-950/90 backdrop-blur-md animate-fade-in">
                ...
                <button
                  ...
                  className="mt-10 bg-emerald-600 hover:bg-emerald-500 text-white px-14 py-5 rounded-[2.5rem] shadow-2xl text-xl font-black tracking-widest active:scale-95 transition-all"
                >
                  受電して開始
                </button>
              </div>
            ...
            {status === 'connecting' && (
              <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-slate-950 animate-fade-in">
                ...
                <p className="font-black text-sky-400 text-sm tracking-widest uppercase">
                  Connecting to AI Client...
                </p>
              </div>
            )}
            <div className="flex-1 p-6 overflow-y-auto space-y-6 custom-scrollbar bg-gradient-to-b from-transparent to-slate-950/30">
              {messages.length === 0 && status === 'connected' && (
                <div className="h-full flex flex-col items-center justify-center opacity-10 grayscale">
                  ...
                  <p className="font-black text-lg uppercase tracking-[0.4em] mt-4">
                    Monitoring Audio
                  </p>
                </div>
              )}
              {messages.map((m, i) => (
                <div
                  key={i}
                  className={`flex flex-col ${m.speaker === 'user' ? 'items-end' : 'items-start'} animate-fade-in`}
                >
                  <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">
                    {m.speaker === 'user' ? traineeName : 'CUSTOMER'}
                  </span>
                  <div
                    className={`max-w-[85%] px-5 py-3 rounded-2xl text-[14px] font-bold leading-relaxed shadow-xl ${m.speaker === 'user' ? 'bg-sky-800 text-white rounded-tr-none' : 'bg-slate-900 text-slate-200 border border-slate-800 rounded-tl-none'}`}
                  >
                    {m.text}
                  </div>
                </div>
